                          ...
                          ITG *filternnzElems, ITG itertop, ITG *fnnzassumed) 
{
    ...
    *filternnz = (ITG)(2LL * sum_dnnz);
    ...
    // === NEW: print first 5 (row, col, val) entries from merged binaries ===
    {
        FILE *fr = fopen("drow.bin", "rb");
        FILE *fc = fopen("dcol.bin", "rb");
        FILE *fv = fopen("dval.bin", "rb");
        if (fr && fc && fv) {
            printf("\nFirst 5 (row, col, val) entries after merge:\n");
            for (int k = 0; k < 5; ++k) {
                int r, c;
                double v;
                size_t nr = fread(&r, sizeof(int),    1, fr);
                size_t nc = fread(&c, sizeof(int),    1, fc);
                size_t nv = fread(&v, sizeof(double), 1, fv);
                if (nr != 1 || nc != 1 || nv != 1) break;
                printf("  %d\t%d\t%.6f\n", r, c, v);
            }
        } else {
            fprintf(stderr, "Could not open merged binaries for preview.\n");
        }
        if (fr) fclose(fr);
        if (fc) fclose(fc);
        if (fv) fclose(fv);
